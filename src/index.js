...
(function initApp(){
    ...
    firebase.auth().onAuthStateChanged(function(user) {
        if (user) {
            // User is signed in.
            const userName = user.displayName ? user.displayName : user.email ? user.email : "User";
            console.log(`%cWelcome :) ${userName}`,"color:green;font-family:system-ui;font-size:2rem;-webkit-text-stroke: 1px black;font-weight:bold");
